...
@pytest.mark.parametrize(
    ("language", "expected_message"),
    [
        (Language.EN, "please confirm your email address"),
        (Language.DE, "bitte bestätigen Sie Ihre E-Mail-Adresse"),
    ],
)
def test_email_verification_backend_translations(language, expected_message):
    assert expected_message in BT.email_verification_subject(language)
