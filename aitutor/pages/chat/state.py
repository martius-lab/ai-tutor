...
class ChatState(SessionState):
            ...
            if exercise:
                ...
                if not self.messages:
                    self.append_chat_message(
                        self.current_exercise.description, role=Role.AITUTOR
                    )
    ...
    async def send_message(self, form_data: dict):
        ...
        if user_message:
            self.append_chat_message(message=user_message, role=Role.USER)
            ...
            self.append_chat_message(message=llm_response, role=Role.AITUTOR)
            self.update_last_user_message_index()
            messages = self.get_messages_dict_gpt()
