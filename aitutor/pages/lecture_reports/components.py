"""Components for the lecture-specific reports page."""
...
from aitutor.pages.lecture_reports.state import (
    LectureReportsState,
...
def show_table_row(table_row: LectureReportTableRow) -> rx.Component:
            ...
            rx.hover_card.root(
                rx.hover_card.trigger(
                    rx.icon_button(
                        "search",
                        size="2",
                        on_click=rx.redirect(table_row.report_view_route),
                        _hover={"cursor": "pointer"},
                    )
                ),
                rx.hover_card.content(
...
def lecture_reports_table():
    """The main table showing all lecture-specific reports."""
    return rx.table.root(
        ...
        rx.table.body(
            rx.foreach(
                LectureReportsState.table_rows,
                show_table_row,
            )
        ),
        variant="surface",
